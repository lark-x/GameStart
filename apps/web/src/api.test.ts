...
test("parseSseBlock preserves event id and multiline data", () => { const value = parseSseBlock("id: cursor-1\nevent: log\ndata: {\"message\":\ndata: \"ok\"}"); assert.equal(value.id, "cursor-1"); assert.equal(value.event, "log"); });
test("parseSseBlock omits absent id for legacy callers", () => { assert.deepEqual(parseSseBlock("data: [DONE]"), { event: "message", done: true }); });
test("parseSseBlock keeps error event ids", () => { const value = parseSseBlock("id: bad-1\nevent: error\ndata: nope"); assert.equal(value.id, "bad-1"); assert.equal(value.event, "error"); });
