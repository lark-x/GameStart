...
import { MessageSquare, Search, Sparkles, Users } from "@lucide/vue";
import type {
  V2ChatContactDto,
...
<template>
  <div class="chat-home">
    <PageHeader
      title="聊天"
      description="选择一个已有角色继续对话，或发起一段新的聊天。角色会使用当前人设与长期记忆。"
    >
      <template #actions>
        <Button variant="secondary" size="md" :loading="loading" @click="load">刷新</Button>
    ...
    <div class="chat-home-search">
      ...
      <Input v-model="search" placeholder="搜索角色或会话..." aria-label="搜索角色或会话" />
    </div>
      ...
      <div class="chat-home-layout">
        ...
        <section class="chat-home-section" aria-labelledby="chat-contacts-title">
          <div class="chat-home-section-head">
            ...
            <h2 id="chat-contacts-title">角色</h2>
          </div>
          <EmptyState
            v-if="filteredContacts.length === 0"
            title="还没有角色"
            description="先在故事里创建一个角色，再开始聊天。"
          >
          ...
          <div v-else class="chat-home-list">
            ...
            >
              ...
              <span class="chat-row-meta">
                ...
                <span v-if="contact.activeMemoryCount > 0" class="chat-row-time">{{ contact.activeMemoryCount }} 条记忆</span>
              </span>
            </button>
          </div>
        </section>
