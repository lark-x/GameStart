...
test("V2 core plugin dispatches every core route through the use-case boundary", async () => {
  ...
  try {
    const requests: Array<{ method: "GET" | "POST" | "PATCH"; url: string; payload?: unknown }> = [
      ...
      { method: "POST", url: "/api/v2/core/runtime/runs/run/saves", payload: { saveId: "save", label: "Checkpoint", idempotencyKey: "save" } },
      { method: "GET", url: "/api/v2/core/worlds/world/runtime/saves" },
