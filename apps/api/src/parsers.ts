...
function parseStoryArcStatus(value: unknown, field: string): NonNullable<CreateStoryArcRequest["status"]> {
  if (value === "DRAFT" || value === "ACTIVE" || value === "ARCHIVED") return value;
  throw new ApiError(400, "BAD_REQUEST", `${field} is invalid`);
}

function parseStoryNodeType(value: unknown, field: string): CreateStoryNodeRequest["nodeType"] {
  if (
    value === "MILESTONE" ||
    value === "TURNING_POINT" ||
    value === "SCENE_SEED" ||
...
function parseStoryNodeStatus(value: unknown, field: string): NonNullable<CreateStoryNodeRequest["status"]> {
  if (
    value === "DRAFT" ||
    value === "PLANNED" ||
    value === "READY" ||
...
function parseStoryNodeTimeMode(value: unknown, field: string): CreateStoryNodeRequest["timeMode"] {
  if (value === "ABSOLUTE" || value === "RELATIVE" || value === "FLOATING") return value;
  throw new ApiError(400, "BAD_REQUEST", `${field} is invalid`);
}

function parseStoryEdgeType(value: unknown, field: string): CreateStoryEdgeRequest["edgeType"] {
  if (
    value === "LEADS_TO" ||
    value === "BRANCHES_TO" ||
    value === "BLOCKS" ||
...
function parsePromptTemplateType(value: unknown, field: string): CreatePromptTemplateRequest["type"] {
  if (
    value === "WORLD" ||
    value === "CHARACTER" ||
    value === "RELATIONSHIP" ||
