...
test("ActiveProfile resolves profile with protocol and model context", async () => { const p={...profile}; delete p.encryptedApiKey; delete p.encryptionIv; const events:ChatObservation[]=[]; const active=new ActiveProfileChatProvider(repo(p),undefined,undefined,(e)=>{ events.push(e); }); await assert.rejects(active.complete(req),/LLM request failed|fetch|timed out/); assert.equal(events[0]?.profileId,"p1"); assert.equal(events[0]?.protocol,LlmProviderProtocol.OPENAI_COMPATIBLE); assert.equal(events[0]?.model,"model-name"); assert.equal(JSON.stringify(events).includes("bad"),false); });
