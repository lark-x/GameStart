...
| 模块 | 当前职责 |
| --- | --- |
| `apps/web` | `/v2` 创作者工作区、HTTP/Mock adapter、Pinia 状态、主题和媒体引用展示 |
| `apps/api` | Fastify 路由、Parser、Use Case、readiness/capabilities、Core/Generation/Media API |
| `apps/worker` | V2 任务派发、场景/资产 Worker、有限重试、租约恢复和本地媒体落盘 |
| `packages/contracts` | V2 请求、响应、错误、Job、Candidate、Release、Runtime 共享契约 |
| `packages/domain` | Canon、Narrative Graph、Typed State、Candidate Review、Release、Play Runtime 规则 |
| `packages/ports` | V2 仓储、快照、候选提交、资产和队列能力接口 |
| `packages/database` | SQLite 连接、顺序 migration、FTS5、V2 仓储映射和事务边界 |
...
Core 分组覆盖 World Canon、Graph、Typed State、Candidate Review、Release、Runtime/Save/Export；Generation 分组覆盖上下文预览、场景 Job、资产 Job 和资产候选审核；Platform 分组提供 health、ready、capabilities 和受控媒体访问。
...
SQLite 保存世界、图、状态 schema、候选、审核、发布包、运行、存档、任务和审计事实；FTS5 只用于本地关键词检索。媒体文件在 `V2_MEDIA_ROOT/v2/assets` 下按内容哈希保存。Redis/BullMQ、Qdrant 均为可重建派生状态。
...
持久本地栈使用 `infra/compose/docker-compose.yml` 的 `api`、`worker`、`redis`、`web` 四个服务，SQLite、Redis 和媒体分别使用命名卷。核心检查：
