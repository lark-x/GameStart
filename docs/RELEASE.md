...
- `V2_SCENE_GENERATION_ENABLED`、`V2_ASSET_GENERATION_ENABLED` 默认 false；未配置外部服务时仍可编辑、发布和游玩。
...
V2 默认使用 `.data/living-network-v2.sqlite`；测试可使用 `V2_SQLITE_PATH=:memory:`。Web 默认使用 HTTP adapter，Mock 只能通过 `VITE_V2_ENABLE_MOCK=true` 显式启用。
...
真实 LLM/ComfyUI 需要显式开关、端点、模型/Workflow 和密钥；真实 Qdrant 需按后续 Slice D 计划验收。未配置时报告“未执行”，不报告为通过。
