...
- V2 的所有固定用户可见文本（包括标题、标签、按钮、空/加载/失败状态、操作反馈和 ARIA 标签）必须使用 `src/v2/locale.ts` 的词条，不得在 Vue 模板或 V2 store 中新增硬编码的自然语言文本。
- 每个新增词条必须同时提供 `en` 与 `zh-CN`；`locale.test.ts` 会校验词条完整性。
- 世界内容、玩家输入、服务端标识符和模型生成内容视为业务数据，保留原始语言；只有 UI 包装文字和本地兜底错误需要翻译。
- 新增 V2 功能的验收必须分别检查英文与中文、语言切换后的即时更新、刷新后的持久化，以及 360px 窄屏下按钮无溢出。